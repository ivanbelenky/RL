...
def _visit_monte_carlo(
    ...
    for n_episode in tqdm(range(n_episodes), desc="Monte Carlo", unit="episodes"):
        ...
        for t, (s_t, a_t, r_tt) in enumerate(sar[::-1]):
            ...
            G = γ * G + r_tt
            if use_N:
                update = _mc_step(v, q, t, s_t, a_t, s, a, n_s, n_sa, G, first_visit)
            else:
                update = _mc_step_α(v, q, t, s_t, a_t, s, a, α, G, first_visit)
...
def off_policy_mc(
    ...
    _check_ranges(
        values=[gamma, n_episodes, max_steps, samples],
...
def _off_policy_monte_carlo(
    ...
    for n_episode in tqdm(
        range(int(n_episodes)), desc="Off-policy MC", unit="episodes"
    ):
        G = 0.0
        s_0, a_0 = MF.random_sa(value=True)
        ...
        for t, (s_t, a_t, r_tt) in enumerate(sar[::-1]):
            ...
            G = γ * G + r_tt
            update = _mc_step_off(
                q, v, t, s_t, a_t, s, a, G, w, c, c_q, first_visit, ordinary
            )
...
def _tdn_on(
    ...
    samples = []
    for n_episode in tqdm(
        range(n_episodes), desc=f"td({n - 1}) variant", unit="episode"
    ):
        s_0, a_0 = _set_s0_a0(MF, s_0, a_0)

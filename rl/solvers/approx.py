...
def gradient_mc(
    ...
    _check_ranges(
        values=[gamma, alpha, n_episodes, max_steps, samples],
...
def _gradient_mc(
    MFS, v_hat, s_0, a_0, alpha, n_episodes, max_steps, tol, optimize, sample_step
):
    α, γ, π = alpha, MFS.gamma, MFS.policy
    q_hat = π.q_hat

    samples, dnorm = [], TOL * 2
    for n_episode in tqdm(range(n_episodes), desc="grad-MC", unit="episodes"):
        if dnorm < tol:
            break
...
def semigrad_tdn(
    ...
    _check_ranges(
        values=[gamma, alpha, n_episodes, max_steps, samples, n],
...
def _semigrad_tdn(
    MFS, v_hat, s_0, a_0, alpha, n, n_episodes, max_steps, tol, optimize, sample_step
):
    """Semi gradient n-step temporal difference
    ...
    samples, dnorm = [], TOL * 2
    for n_episode in tqdm(range(n_episodes), desc="semigrad-TD", unit="episodes"):
        if dnorm < tol:
            break
...
def _diff_semigrad(MFS, v_hat, s_0, a_0, alpha, beta, n, T, tol, optimize, sample_step):
    """
    ...
    R, A, S, avg_R = [], [a], [s], 0
    for t in tqdm(range(T), desc="semigrad-TD", unit="episodes"):
        if dnorm < tol:
            break
...
def semigrad_td_lambda(
    ...
    _check_ranges(
        values=[alpha, gamma, lambdaa, n_episodes, samples],
...
def reinforce_mc(
    ...
    _typecheck_all(
        transition=transition,
        constants=[alpha, gamma, n_episodes, samples, tol],
        policies=[policy],
    )

    _check_ranges(
        values=[alpha, gamma, n_episodes, samples],
        ranges=[(0, 1), (0, 1), (1, np.inf), (1, 1001)],
    )
...
def _reinforce_mc(MFS, s_0, a_0, alpha, n_episodes, max_steps, tol, sample_step):
    ...
    for n_episode in tqdm(
        range(n_episodes), desc="MC Policy Gradient", unit="episodes"
    ):
        s, a = _set_s0_a0(MFS, s_0, a_0)
